...
class PlayState extends MusicBeatState {
	...
	public var player2Strums:FlxTypedGroup<FlxSprite>;
	...
	override public function create() {
		...
		perfectMode = PreferencesMenu.getPref('botplay');
	...
	override public function update(elapsed:Float) {
		...
		if (generatedMusic) {
			notes.forEachAlive(function(daNote:Note) {
				...
				if (!daNote.mustPress && daNote.wasGoodHit) {
					...
					if (PreferencesMenu.getPref('player2Strums'))
					{
						player2Strums.forEach(function(spr:FlxSprite)
						{
				...
				if (doKill) {
					if (daNote.tooLate || !daNote.wasGoodHit) {
			            ...
			            boyfriend.playAnim("sing" + ManiaBullshit.singAnims.get(ManiaBullshit.anims[SONG.mania][daNote.noteData]).toUpperCase() + "miss", true);
					    }
					}
		...
		if (!inCutscene && !customInput && !perfectMode)
			keyShit();
		else if (!inCutscene && !customInput && perfectMode)
			botKeyShit();
	...
	function endSong():Void {
		...
		vocals.volume = 0;
		if (SONG.validScore || !perfectMode) {
			#if !switch
	...
	private function botKeyShit():Void{
		...
		if (boyfriend.holdTimer > 0.004 * Conductor.stepCrochet * boyfriend.singDuration
	...
	public function updateTime() {
		...
		timeTxt.text = FlxStringUtil.formatTime(FlxG.sound.music.time / 1000.0)+" / "+FlxStringUtil.formatTime(FlxG.sound.music.length / 1000.0);
		timeTxt.screenCenter(X);
		timeTxt.x -= 30;
	}
